...
查询间隔默认 5 秒（建议 3-5 秒），实际间隔在 `delay×(1±jitter-ratio)` 间随机抖动，默认 ±50%（约 2.5-7.5 秒），避免固定频率被识别；最后一条查询完成后不再等待。

触发限流后的处理（CLI 与 Web 一致）：

1. **长冷却自动重试**：单条内按约 60 / 120 / 180 秒冷却重试（默认最多 3 次，等待期间可在 Web 端随时取消）；HTTP 5xx、超时、网络异常仍走短退避（10/20/40 秒）；4xx 立即失败不重试。
2. **熔断**：连续 3 条限流即暂停本轮，避免在封禁期持续请求加重限制。
3. **当轮补查**：本轮结束前等待 60 秒，对限流条目慢速补查一次。
4. **失败标注与续跑**：最终仍未完成的行，`ndls状态` 标注「限流，未完成，请重新运行续查」；成功条目已存进度，重新运行（CLI）或再次点击「开始查询」（Web）会跳过已完成、只补剩余条目。
